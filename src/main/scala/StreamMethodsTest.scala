object StreamMethodsTest extends App{
  val myStream = Stream.apply(0,1,2,3,4,5)
  println(myStream.map(_*2).toList)
  println(myStream.filter(_%2==1).toList)
  println(myStream.flatMap(x=>Stream.apply(x,x+1)).toList)
}
